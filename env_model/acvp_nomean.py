...
def train(game):
    ...
    def pre_process(x):
    	bsz, dep, _, _ = x.shape
    	x = np.array([resize(x[i, :, :, :], (dep, 84, 84), preserve_range=True) for i in range(bsz)]).astype(np.uint8)
    	x = np.array([resize(x[i, :, :, :], (dep, 210, 160), preserve_range=True) for i in range(bsz)]).astype(np.uint8)
        if x.shape[1] == 12:
            ...
            while not batcher.end():
                if iteration % 100 == 0:
                    ...
                    for test_ep in tqdm(test_indices):                
                        ...
                        #     np.save('data_nomean/acvp-sample/sample_input', x)
                        #     np.save('data_nomean/acvp-sample/sample_processed_input', pre_process(x))
                        ...
                        if test_ep == ep_to_print:
                            ...
                            x, a, y = test_batcher.next_batch()
                            x = pre_process(x)
                            preds = net.predict(x, a)
                            y_ = post_process(preds)
                            torchvision.utils.save_image(torch.from_numpy(y_), 'data_nomean/acvp-sample/%s-%09d.png' % (game, iteration))
                            torchvision.utils.save_image(torch.from_numpy(y), 'data_nomean/acvp-sample/%s-%09d-truth.png' % (game, iteration))
                            np.save('data_nomean/acvp-sample/%s-%09d' % (game, iteration), y_)

                    logger.info('Iteration %d, test loss %f' % (iteration, np.mean(losses)))
                    torch.save(net.state_dict(), 'data_nomean/acvp-%s.bin' % (game))
