...
class EnvironmentModel(object):
    def __init__(self, config):
        ...
        with tf.variable_scope(config.scope):
            ...
            upconv2 = tf.pad(upconv2, tf.constant([[0, 0], [1, 1], [0, 0], [0, 0]]))
            # print('upconv2', upconv2.get_shape())
            upconv1 = layers.conv2d_transpose(upconv2, config.n_stacked, 8, stride=2, activation_fn=None, padding='valid')
    ...
    while True:
        ...
        for ep in indices_train:
            ...
            while not batcher.end():
                if iteration % 100 == 0:
                    ...
                    for test_ep in tqdm(test_indices):
                        ...
                        if test_ep == ep_to_print:
                            ...
                            np.save('data_reg/acvp-sample/%s-%09d' % (game, iteration), preds)
                ...
                if iteration % 100 == 0:
                    logger.info('Iteration reg %d, loss %f' % (iteration, loss))
