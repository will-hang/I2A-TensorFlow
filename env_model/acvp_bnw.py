                ...
                if iteration % 100 == 0:
                    ...
                    for test_ep in tqdm(test_indices):
                        ...
                        if test_ep == ep_to_print:
                            ...
                            preds = net.predict(pre_process(x), a)
                            y_, y__y = post_process(preds)
                            torchvision.utils.save_image(torch.from_numpy(y_), 'data_pyt/acvp-sample/%s-%09d.png' % (game, iteration))
                            torchvision.utils.save_image(torch.from_numpy(y__y), 'data_pyt/acvp-sample/%s-%09dpoopybutt.png' % (game, iteration))
                            torchvision.utils.save_image(torch.from_numpy(y), 'data_pyt/acvp-sample/%s-%09d-truth.png' % (game, iteration))
                            np.save('data_pyt/acvp-sample/%s-%09d' % (game, iteration), preds)
