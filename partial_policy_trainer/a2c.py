...
class Runner(object):
    ...
    def run(self):
        ...
        return mb_obs, mb_states, mb_rewards, mb_masks, mb_actions, mb_values, ep_reward_means
...
def learn(policy, env, seed, nsteps=5, total_timesteps=int(80e6), vf_coef=0.5, ent_coef=0.01, max_grad_norm=0.5, lr=7e-4, lrschedule='linear', epsilon=1e-5, alpha=0.99, gamma=0.99, log_interval=100):
    ...
    file_writer = tf.summary.FileWriter('results/', tf.Session().graph)
    nbatch = nenvs*nsteps
    tstart = time.time()
    ...
    for update in range(1, total_timesteps//nbatch+1):
        obs, states, rewards, masks, actions, values, ep_reward_means = runner.run()
